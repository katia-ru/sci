...
#output_file = "/home/katia/Science/hemoglobin/ver2_1MBN/for_Michail"
        ...
        for jj in xrange(ii+1, N_OX):
            if ii != jj :
                ...
                if dist_oo < R_BOND_oo:
                    ...
                    if (dist1 < R_BOND_oh):
                        EDGES += 1
                        list_edges.append([ii, jj])
                        
                    elif (dist2 < R_BOND_oh):
                        EDGES += 1
                        list_edges.append([ii, jj])
    out_file.write(str(N_OX)+"\t"+str(EDGES)+"\n")
    print EDGES
    for ii in list_edges:
        out_file.write(str(ii[0])+"\t"+str(ii[1])+"\n")

def function_cycles_distribution(cycles):
    N = 7
    in_file = open(cycles, "r")
    oo, h_1, h_2 = for_read()
    ow_x_coord, ow_y_coord, ow_z_coord = [i[0] for i in oo], [i[1] for i in oo], [i[2] for i in oo]
    hw1_x_coord, hw1_y_coord, hw1_z_coord = [i[0] for i in h_1], [i[1] for i in h_1], [i[2] for i in h_1]
    hw2_x_coord, hw2_y_coord, hw2_z_coord = [i[0] for i in h_2], [i[1] for i in h_2], [i[2] for i in h_2]
    ...
    for ii in xrange(N_OX):
#        print ii
        dict_numbers_oo[ii] = [ow_x_coord[ii], ow_y_coord[ii], ow_z_coord[ii]]
        dict_numbers_h1[ii] = [hw1_x_coord[ii], hw1_y_coord[ii], hw1_z_coord[ii]]
        dict_numbers_h2[ii] = [hw2_x_coord[ii], hw2_y_coord[ii], hw2_z_coord[ii]]
    for current in in_file:
        verts = current.split()
        if len(verts) == N:
            for ii in verts:
                print 1111111111
                for jj in verts[verts.index(ii)+1:]:
                    ii, jj = int(ii), int(jj)
                    dist_oo = math.sqrt((dict_numbers_oo[ii][0] - dict_numbers_oo[jj][0])**2 + \
                    (dict_numbers_oo[ii][1] - dict_numbers_oo[jj][1])**2 + (dict_numbers_oo[ii][2] - dict_numbers_oo[jj][2])**2)
                    dist_oo = int(dist_oo/width)*width
                    
                    if dist_oo < R_BOND_oo:
                        dist1 = math.sqrt((dict_numbers_oo[ii][0] - dict_numbers_h1[ii][0])**2 + \
                        (dict_numbers_oo[ii][1]-dict_numbers_h1[ii][1])**2 + (dict_numbers_oo[ii][2]-dict_numbers_h1[ii][2])**2)
                
                        dist2 = math.sqrt((dict_numbers_oo[ii][0] - dict_numbers_h2[ii][0])**2 + \
                        (dict_numbers_oo[ii][1]-dict_numbers_h2[ii][1])**2 + (dict_numbers_oo[ii][2]-dict_numbers_h2[ii][2])**2)
                        if (dist1 < R_BOND_oh):
...
function_cycles_distribution("/home/katia/Science/ver2_scripts/out_qmbn")
